  ...
  var JavaScriptLanguage = /** @class */ (function () {
    ...
    JavaScriptLanguage.prototype.readExpression = function () {
      ...
      if (text === "function") {
        ...
        return "keyword";
      } else if (text === "var" || text === "let" || text === "const") {
        this.push({ type: this.readVariable, state: "none" });
        return "keyword";
      }
      if (this._token.type === this.readStatement) {
        if (
          this._blockStatements.hasOwnProperty(text) ||
          this._terminalStatements.hasOwnProperty(text)
        ) {
          return "keyword";
      ...
      if (
        this._locals.hasOwnProperty(text) ||
        this._operators.hasOwnProperty(text)
      ) {
        this._token.regexp = false;
        return "keyword";
    ...
    JavaScriptLanguage.prototype.readBlockStatement = function () {
      if (this._textReader.match("}")) {
        if (
          this._token.type === this.readBlockStatement &&
          this._token.state !== "root"
        ) {
          this.pop();
          return "punctuation";
        }
    ...
    JavaScriptLanguage.prototype.readObject = function () {
      if (this._textReader.match("}")) {
        ...
        this._textReader.skipLineTerminators()
      ) {
        return "text";
      } else if (this._textReader.match(",")) {
    ...
    JavaScriptLanguage.prototype.matchRegExp = function () {
      ...
      if (this._token.regexp && this._textReader.match("/")) {
        while (
          this._textReader.peek().length > 0 &&
          !this._textReader.skipLineTerminators()
        ) {
          if (this._textReader.match("\\")) {
            this._textReader.read();
    ...
    JavaScriptLanguage.prototype.popToken = function (tokenType) {
      if (
        this._tokenStack.length > 1 &&
        this._tokenStack[this._tokenStack.length - 1].type === tokenType
      ) {
        this.pop();
      }
    };
